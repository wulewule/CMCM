...
def test():
    ...
    unet_model.load_state_dict(torch.load("ldmc_dfmodel.ckpt"))
    class_model.load_state_dict(torch.load("ldmc_classmodel.ckpt"))
    unet_model.eval()
    class_model.eval()
    
    image = np.load("PET-CT_data/001_26.npz")['a']
    imagb = np.load("PET-CT_data/001_26.npz")['b']
    ...
    print(image.shape, imagb.shape)
    
    image[image>20000]=20000
    image = min_max(image, 0, 20000)
    image = cv2.resize(image, (256,256), cv2.INTER_AREA)
    image = np.stack([image, image, image], axis=0)[None,:,:,:]

    image = torch.tensor(image).to(memory_format=torch.contiguous_format).float()
    image = image.to(device, dtype=torch.float)
    
    xc = torch.tensor(1*[0])
    c = class_model(xc.to(device))
    
    x = first_model.encode_first(image).mode()
    t = torch.randint(0, num_timesteps, (x.shape[0],), device=device).long()
    noise =None
    noise = default(noise, lambda: torch.randn_like(x)).to(device)
    x_noisy = q_sample(x_start=x, t=t, noise=noise)

    output = first_model.decode_first(unet_model(x, t, c))
    output = d_min_max(output, -1024, 2000)
    print(output.shape, imagb.shape)
    
    
    imagb = cv2.normalize(imagb,None,0,255,cv2.NORM_MINMAX).astype(np.uint8)
    imagb = cv2.resize(imagb, (256,256), cv2.INTER_AREA)
    ...
    print(output.shape, imagb.shape)
    psnr = compare_psnr(output[:,:,0], imagb)
    ssim = compare_ssim(output[:,:,0], imagb)
    print(psnr, ssim)
    cv2.imwrite('haa.png', output)
    cv2.imwrite('ha.png', imagb)
