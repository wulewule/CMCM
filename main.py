...
class Transfer(Dataset):
    def __init__(self,
        ...
        self.image_paths = self.image_paths2
        #self.middle_paths = self.image_paths2
...
def main():
    ...
    unet_model  = UNetModel(
        ...
        dropout=0.6,
        channel_mult=(1,2,4),
        num_head_channels=16,).to(device)
    
    #unet_model.load_state_dict(torch.load("cailast_dfmodel.ckpt"))
    
    optimizer_G = optim.AdamW(unet_model.parameters(), lr=lr_G, weight_decay=0.01)
    scheduler = lr_scheduler.CosineAnnealingLR(optimizer_G, T_max=int(12800/16*400), eta_min=1e-6)
    
    with torch.no_grad():
        unet_model.eval()
        X, Y, _ = next(iter(valid_loader))
        ...
        g = first_model.decode_first(unet_model(x_noisy, t))
    ...
    for epoch in range(epochs):
        ...
        for step, (X, Y, _) in train_pbar:
            unet_model.zero_grad() 
            
            x = first_model.encode_first(X.to(device, dtype=torch.float)).mode()
            y = first_model.encode_first(Y.to(device, dtype=torch.float)).mode()
            t = torch.randint(0, num_timesteps, (x.shape[0],), device=device).long()
        ...
        with torch.no_grad():
            ...
            if (epoch+1)%10 == 0:
                ...
                #if ssim > max_ssim:
                    #max_ssim = ssim
                    #torch.save(unet_model.state_dict(), 'caibest_dfmodel.ckpt')
                    #torch.save(class_model.state_dict(), 'caibest_classmodel.ckpt')
        
        torch.save(unet_model.state_dict(), 'ldmbunoisy_dfmodel.ckpt')
        #torch.save(class_model.state_dict(), 'cailast_classmodel.ckpt')
        torch.cuda.empty_cache()
